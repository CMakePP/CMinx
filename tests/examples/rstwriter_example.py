...
# Add a doctest for both showing usage and to ensure future revisions
# still work the same.
...
# Images can also have options, here we change the image width and height,
# stretching it
img.option("width", 500)
img.option("height", 100)

# Adding a subsection to the root (appears after John's admonition, social
# network, and image)
s = writer.section('A subsection')
s.text("Subsection's content")

# Adding lists. These use varargs so if you have a prebuilt list already
# you'll want to splat it out
